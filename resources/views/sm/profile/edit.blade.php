...
@section('content')
  ...
  <div class="container">
    ...
    <form action="{{ url('/sm/profile/edit/'.auth()->user()->id) }}" method="POST" enctype="multipart/form-data">
      @method('patch')
      @csrf
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-5">
        <a href="{{ url('/sm/profile/' . auth()->user()->nama) }}" class="btn btn-secondary btn-user">Kembali</a>
        <button type="submit" name="ubah" class="btn btn-primary btn-user" onclick="return confirm('Yakin?')">Ubah Data</button>
      </div>
    </div>
  </form>
  </div>
